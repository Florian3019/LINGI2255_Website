Meteor.methods({
	 ...
	 * This is magic
	 */
	'populateDB': function(tournamentDateTab, nTypesTab, nAlonesTab, nUnregistered, nCourtSaturdayTab, nCourtSundayTab, nCourtBothTab, nStaff, nAdmin) {
        ...
        function createPair(type, category, alone, tournamentDate, count) {
			...
			if (!alone) {
				...
				pairID = Meteor.call("updatePair", {player1: {_id:userID1, playerWish:wishes1[0], courtWish:wishes1[1], otherWish:wishes1[2], extras:extra1}, player2: {_id:userID2, playerWish:wishes2[0], courtWish:wishes2[1], otherWish:wishes2[2], extras:extra2}, year:tournamentYear.toString()});
			}
			else {
				pairID = Meteor.call("updatePair", {player1: {_id:userID1, playerWish:wishes1[0], courtWish:wishes1[1], otherWish:wishes1[2], extras:extra1}, year:tournamentYear.toString()});
			}
			Meteor.call("addPairToTournament", pairID, tournamentYear.toString(), dateMatch);
        } // End createPair


		function popDBForYear(tournamentData, nTypes, nAlones, nCourtSaturday, nCourtSunday, nCourtBoth) {
		...
		*/
		...
		for (var i=0; i<tournamentDateTab.length; i++) {
			popDBForYear(tournamentDateTab[i], nTypesTab[i], nAlonesTab[i], nCourtSaturdayTab[i], nCourtSundayTab[i], nCourtBothTab[i]);
		}
